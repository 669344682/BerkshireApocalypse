...
function Teamsystem.openTeampanel(player)
	if(getElementData(player,"Team") >= 1)then
		...
		triggerClientEvent(player,"Teamsystem.openTeampanel",player,name);
...
function Teamsystem.updateTeampanel(player)
	...
	local cash = getPlayerData("Teams","ID",team,"Cash");
	local name = getPlayerData("Teams","ID",team,"Name");
	triggerClientEvent(player,"Teamsystem.updateTeampanel",player,Teamsystem.getMember(team),cash,name);
end
